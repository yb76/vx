...
#ifdef USE_MYSQL
	char * user = "iris";
	char * password = "th1nkr1s";
	char * port = "3306";
#else
	char * user = "postgres";
	char * password = "ristest";
	char * port = "5432";
#endif
...
void import(char * schema, char * sourceFile)
{
	...
	sprintf(connectString, "host=%s port=%s dbname=%s user=%s connect_timeout=5", host, port, schema, user);
...
int main(int argc, char ** argv)
{
	...
	while((arg = getopt(argc, argv, "i:o:u:U:k:h?")) != -1)
	{
		switch(arg)
		{
			case 'i':
			...
			case '?':
			default:
				printf(	"Usage: %s [-h=help] [-?=help] [-i databaseIPAddress=localhost] [-o databasePortNumber=5432] [-u user=postgres] [-U password=ristest] [-k unix_socket=NULL] schema [file]\n", argv[0]);
				exit(-1);
		}
	}
	...
	else importJSON(argv[optind], argv[optind+1]);
#else
	else
	{
		int i;
